...
//const ipcBus = new events.EventEmitter()
const proc = new events.EventEmitter()
// eval in window scope:
// http://www.ecma-international.org/ecma-262/5.1/#sec-10.4.2
const geval = eval

console.log('before loading IpcBus')
const IpcBus = require('../common/api/ipc-bus')
console.log('after loading IpcBus')
console.log('before creating IpcBus => ' + process.type)
const ipcBus = IpcBus(ipcRenderer, "renderer");
const util = require('util')
console.log('after creating IpcBus: ' + util.inspect(ipcBus))
...
let preloadSandbox = `(function(require, process, Buffer, global) {
const { ipcRenderer, ipcBus } = require('electron');
...
preloadSandboxFn(preloadRequire, proc, Buffer, global)
